...
public class SanFunc implements Factor{ //保留Factor元素做类型判断
    ...
    public HashSet<Values> getValues() {
        ...
        if (!sin && new calculator().addValue(exprValues, zero, false).isEmpty() && !power.equals(BigInteger.ZERO)) {
            values.add(new Values(new ZeroInt(BigInteger.ONE)));
        } else if (sin && new calculator().addValue(exprValues, zero, true).isEmpty() && !power.equals(BigInteger.ZERO)) {
